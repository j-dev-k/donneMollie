...
class Dmm_Admin {
    ...
    public function dmm_page_donations()
    {
        ...
        {
            try {
                $payment = $mollie->get('payments/' . $_GET['payment']);
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['payment']) && check_admin_referer('delete-donation_' . $_GET['payment']))
        {
            ...
            if ($update)
                wp_redirect('?page=' . $_REQUEST['page'] . '&msg=delete-ok');
            else
                wp_redirect('?page=' . $_REQUEST['page'] . '&msg=delete-nok');
        }
        ...
        <div class="wrap">
            ...
            <a style="float: right;" href="<?php echo admin_url('admin-post.php?action=dmm_export' . (isset($_GET['subscription']) ? '&subscription=' . $_GET['subscription'] : '') . (isset($_GET['search']) ? '&search=' . $_GET['search'] : ''));?>"><?php esc_html_e('Export', 'doneren-met-mollie') ?></a>
    ...
    public function dmm_page_donors()
    {
        if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['customer']) && check_admin_referer('delete-donor_' . $_GET['customer']))
        {
            ...
            try {
                $mollie->delete('customers/' . $_GET['customer']);
    ...
    public function dmm_page_subscriptions()
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'cancel' && isset($_GET['subscription']) && check_admin_referer('cancel-subscription_' . $_GET['subscription']))
        {
            ...
            try {
                $cancelledSub = $mollie->delete('customers/' . $customer->customer_id . '/subscriptions/' . $_GET['subscription']);
            } catch (Exception $e) {
                wp_redirect('?page=' . $_REQUEST['page'] . '&msg=cancel-nok&status=unknown');
            }
                ...
                <tbody>
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp">
                            ...
                            <small><?php esc_html_e('If enabled, the donor can choose their own currency for open amount donations', 'doneren-met-mollie');?></small>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc">
