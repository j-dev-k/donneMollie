...
class Dmm_Webhook {
    ...
    protected function handle_request($query)
    {
        ...
        do_action('dmm_webhook_called', $_POST['id']);
        ...
        try {
            ...
            if (!$query->query_vars['sub'])
            {
                ...
                $payment = $mollie->get('payments/' . $payment_id);
                ...
                if (($query->query_vars['first'] && $query->query_vars['secret']) && $status == 'paid')
                {
                    ...
                    $subscription = $mollie->post('customers/' . $customer->customer_id . '/subscriptions', array(
                        "amount"        => array(
